...
declare
  ...
  nadia constant uuid := '00000000-0000-0000-0000-0000000000a3';
  omar  constant uuid := '00000000-0000-0000-0000-0000000000b4';
  petra constant uuid := '00000000-0000-0000-0000-0000000000c5';
  quinn constant uuid := '00000000-0000-0000-0000-0000000000d6';
  people jsonb := jsonb_build_object('alice', alice, 'bob', bob, 'carol', carol, 'dave', dave,
    ...
    'nadia', nadia, 'omar', omar, 'petra', petra, 'quinn', quinn);
  who_key text;
  tests jsonb := $j$[
    ...
    {"n":"profiles: alice completes her own profile (remote, France)","as":"alice","sql":"update public.profiles set display_name='Alice', work_modes=array['remote'], country='FR', idea_statuses=array['exploring'], weekly_hours=array['5_10'], ambitions=array['for_fun'], visibility='private', onboarded=true where id='{alice}'","expect":"rows=1"},
    {"n":"profiles: bob completes his own profile with several answers per question (remote, Lebanon)","as":"bob","sql":"update public.profiles set display_name='Bob', work_modes=array['remote'], country='LB', idea_statuses=array['wants_to_join','open_to_merge'], weekly_hours=array['10_20','20_plus'], ambitions=array['side_income','full_time'], visibility='private', onboarded=true where id='{bob}'","expect":"rows=1"},
    ...
    {"n":"feed setup: complete the 25 profiles (remote, France)","as":"admin","sql":"update public.profiles set display_name='Bulk', work_modes=array['remote'], country='FR', idea_statuses=array['exploring'], weekly_hours=array['5_10'], ambitions=array['for_fun'], visibility='private', onboarded=true where id in (select id from auth.users where email like 'bulk%@test.invalid')","expect":"rows=25"},
    ...
    {"n":"zones: setup: complete their profiles and pick video games","as":"admin","sql":"with done as (update public.profiles p set display_name=v.n, work_modes=array['local'], country='FR', city=v.city, postal_code=v.pc, idea_statuses=array['exploring'], weekly_hours=array['5_10'], ambitions=array['for_fun'], visibility='private', onboarded=true from (values ('{paris}'::uuid,'Paris','Paris','75001'), ('{boulogne}'::uuid,'Boulogne','Boulogne-Billancourt','92100'), ('{lyona}'::uuid,'LyonA','Lyon','69003'), ('{lyonb}'::uuid,'LyonB','Villeurbanne','69100')) as v(id,n,city,pc) where p.id=v.id returning p.id) insert into public.profile_categories (profile_id, category_id) select d.id, c.id from done d cross join public.categories c where c.slug='video_games'","expect":"rows=4"},
    ...
    {"n":"projects: alice creates a project (active category, all required fields)","as":"alice","sql":"insert into public.projects (owner_id, name, category_id, started_on, ended_on, outcome, lessons) select '{alice}', 'Jeu indé solo', (select id from public.categories where slug='video_games'), '2024-01-01', '2024-06-01', 'launched_then_stopped', 'Le marché des jeux indé est trop encombré pour un solo.'","expect":"rows=1"},
    {"n":"projects: an inactive category is refused","as":"alice","sql":"insert into public.projects (owner_id, name, category_id, started_on, ended_on, outcome, lessons) select '{alice}', 'x', (select id from public.categories where slug='test_inactive'), '2024-01-01', '2024-06-01', 'launched_then_stopped', 'Le marché des jeux indé est trop encombré pour un solo.'","expect":"error=row-level security"},
    {"n":"projects: empty lessons is refused (what you learned is required)","as":"alice","sql":"insert into public.projects (owner_id, name, category_id, started_on, ended_on, outcome, lessons) select '{alice}', 'x', (select id from public.categories where slug='video_games'), '2024-01-01', '2024-06-01', 'launched_then_stopped', ''","expect":"error=projects_lessons_check"},
    {"n":"projects: lessons over 500 characters is refused","as":"alice","sql":"insert into public.projects (owner_id, name, category_id, started_on, ended_on, outcome, lessons) select '{alice}', 'x', (select id from public.categories where slug='video_games'), '2024-01-01', '2024-06-01', 'launched_then_stopped', ''","expect":"error=projects_lessons_check"},
    {"n":"projects: an empty name is refused","as":"alice","sql":"insert into public.projects (owner_id, name, category_id, started_on, ended_on, outcome, lessons) select '{alice}', '', (select id from public.categories where slug='video_games'), '2024-01-01', '2024-06-01', 'launched_then_stopped', 'Le marché des jeux indé est trop encombré pour un solo.'","expect":"error=projects_name_check"},
    {"n":"projects: a name over 80 characters is refused","as":"alice","sql":"insert into public.projects (owner_id, name, category_id, started_on, ended_on, outcome, lessons) select '{alice}', 'xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx', (select id from public.categories where slug='video_games'), '2024-01-01', '2024-06-01', 'launched_then_stopped', 'Le marché des jeux indé est trop encombré pour un solo.'","expect":"error=projects_name_check"},
    {"n":"projects: an invalid outcome is refused","as":"alice","sql":"insert into public.projects (owner_id, name, category_id, started_on, ended_on, outcome, lessons) select '{alice}', 'x', (select id from public.categories where slug='video_games'), '2024-01-01', '2024-06-01', 'went_viral', 'Le marché des jeux indé est trop encombré pour un solo.'","expect":"error=projects_outcome_check"},
    {"n":"projects: outcome 'ongoing' with an end date is refused","as":"alice","sql":"insert into public.projects (owner_id, name, category_id, started_on, ended_on, outcome, lessons) select '{alice}', 'x', (select id from public.categories where slug='video_games'), '2024-01-01', '2024-06-01', 'ongoing', 'Le marché des jeux indé est trop encombré pour un solo.'","expect":"error=projects_ended_on_matches_outcome"},
    {"n":"projects: an outcome other than 'ongoing' needs an end date","as":"alice","sql":"insert into public.projects (owner_id, name, category_id, started_on, ended_on, outcome, lessons) select '{alice}', 'x', (select id from public.categories where slug='video_games'), '2024-01-01', null, 'launched_then_stopped', 'Le marché des jeux indé est trop encombré pour un solo.'","expect":"error=projects_ended_on_matches_outcome"},
    {"n":"projects: the end date cannot be before the start date","as":"alice","sql":"insert into public.projects (owner_id, name, category_id, started_on, ended_on, outcome, lessons) select '{alice}', 'x', (select id from public.categories where slug='video_games'), '2024-06-01', '2024-01-01', 'launched_then_stopped', 'Le marché des jeux indé est trop encombré pour un solo.'","expect":"error=projects_ended_on_order"},
    {"n":"projects: dates are month precision only (the 15th is refused)","as":"alice","sql":"insert into public.projects (owner_id, name, category_id, started_on, ended_on, outcome, lessons) select '{alice}', 'x', (select id from public.categories where slug='video_games'), '2024-01-15', '2024-06-01', 'launched_then_stopped', 'Le marché des jeux indé est trop encombré pour un solo.'","expect":"error=projects_started_on_check"},
    {"n":"projects: an invalid weekly-hours bucket is refused","as":"alice","sql":"insert into public.projects (owner_id, name, category_id, started_on, ended_on, outcome, lessons, hours_per_week) select '{alice}', 'x', (select id from public.categories where slug='video_games'), '2024-01-01', '2024-06-01', 'launched_then_stopped', 'Le marché des jeux indé est trop encombré pour un solo.', '30_plus'","expect":"error=projects_hours_per_week_check"},
    {"n":"projects: a role over 80 characters is refused","as":"alice","sql":"insert into public.projects (owner_id, name, category_id, started_on, ended_on, outcome, lessons, role) select '{alice}', 'x', (select id from public.categories where slug='video_games'), '2024-01-01', '2024-06-01', 'launched_then_stopped', 'Le marché des jeux indé est trop encombré pour un solo.', 'xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx'","expect":"error=projects_role_check"},
    {"n":"projects: a SIRET that is not 14 digits is refused","as":"alice","sql":"insert into public.projects (owner_id, name, category_id, started_on, ended_on, outcome, lessons, siret) select '{alice}', 'x', (select id from public.categories where slug='video_games'), '2024-01-01', '2024-06-01', 'launched_then_stopped', 'Le marché des jeux indé est trop encombré pour un solo.', '1234'","expect":"error=projects_siret_check"},
    {"n":"projects: a valid 14-digit SIRET is accepted","as":"alice","sql":"update public.projects set siret='73282932000074' where id=(select id from public.projects where name='Jeu indé solo' and owner_id='{alice}')","expect":"rows=1"},
    {"n":"projects: alice can read her own project","as":"alice","sql":"select 1 from public.projects where id=(select id from public.projects where name='Jeu indé solo' and owner_id='{alice}')","expect":"rows=1"},
    {"n":"projects: alice can edit her own project (a field that doesn't change the lookup key)","as":"alice","sql":"update public.projects set role='Fondateur solo' where id=(select id from public.projects where name='Jeu indé solo' and owner_id='{alice}')","expect":"rows=1"},
    {"n":"projects: moving a project to an inactive category is refused","as":"alice","sql":"update public.projects set category_id=(select id from public.categories where slug='test_inactive') where id=(select id from public.projects where name='Jeu indé solo' and owner_id='{alice}')","expect":"error=row-level security"},
    {"n":"projects: bob cannot see alice's project","as":"bob","sql":"select 1 from public.projects where id=(select id from public.projects where name='Jeu indé solo' and owner_id='{alice}')","expect":"rows=0"},
    {"n":"projects: bob cannot edit alice's project","as":"bob","sql":"update public.projects set role='hacked' where id=(select id from public.projects where name='Jeu indé solo' and owner_id='{alice}')","expect":"rows=0"},
    {"n":"projects: bob cannot delete alice's project","as":"bob","sql":"delete from public.projects where id=(select id from public.projects where name='Jeu indé solo' and owner_id='{alice}')","expect":"rows=0"},
    {"n":"projects: bob cannot create a project as alice","as":"bob","sql":"insert into public.projects (owner_id, name, category_id, started_on, ended_on, outcome, lessons) select '{alice}', 'x', (select id from public.categories where slug='video_games'), '2024-01-01', '2024-06-01', 'launched_then_stopped', 'Le marché des jeux indé est trop encombré pour un solo.'","expect":"error=row-level security"},
    {"n":"projects: logged-out visitor cannot read projects","as":"anon","sql":"select 1 from public.projects","expect":"error=permission denied"},
    {"n":"projects: logged-out visitor cannot create a project","as":"anon","sql":"insert into public.projects (owner_id, name, category_id, started_on, ended_on, outcome, lessons) select '{alice}', 'x', (select id from public.categories where slug='video_games'), '2024-01-01', '2024-06-01', 'launched_then_stopped', 'Le marché des jeux indé est trop encombré pour un solo.'","expect":"error=permission denied"},
    {"n":"project_links: alice adds a link to her project","as":"alice","sql":"insert into public.project_links (project_id, label, url) values ((select id from public.projects where name='Jeu indé solo' and owner_id='{alice}'), 'Notre site', 'https://exemple.fr')","expect":"rows=1"},
    {"n":"project_links: an http link is refused","as":"alice","sql":"insert into public.project_links (project_id, label, url) values ((select id from public.projects where name='Jeu indé solo' and owner_id='{alice}'), 'Non', 'http://exemple.fr')","expect":"error=project_links_url_https"},
    {"n":"project_links: a javascript link is refused","as":"alice","sql":"insert into public.project_links (project_id, label, url) values ((select id from public.projects where name='Jeu indé solo' and owner_id='{alice}'), 'Non', 'javascript:alert(1)')","expect":"error=project_links_url_https"},
    {"n":"project_links: an empty label is refused","as":"alice","sql":"insert into public.project_links (project_id, label, url) values ((select id from public.projects where name='Jeu indé solo' and owner_id='{alice}'), '', 'https://exemple.fr')","expect":"error=project_links_label_check"},
    {"n":"project_links: bob cannot read alice's links","as":"bob","sql":"select 1 from public.project_links where project_id=(select id from public.projects where name='Jeu indé solo' and owner_id='{alice}')","expect":"rows=0"},
    {"n":"project_links: bob cannot add a link to alice's project","as":"bob","sql":"insert into public.project_links (project_id, label, url) values ((select id from public.projects where name='Jeu indé solo' and owner_id='{alice}'), 'Intrus', 'https://exemple.fr')","expect":"error=row-level security"},
    {"n":"project_links: bob cannot delete alice's link","as":"bob","sql":"delete from public.project_links where project_id=(select id from public.projects where name='Jeu indé solo' and owner_id='{alice}')","expect":"rows=0"},
    {"n":"project_links: alice deletes her own link","as":"alice","sql":"delete from public.project_links where project_id=(select id from public.projects where name='Jeu indé solo' and owner_id='{alice}')","expect":"rows=1"},
    {"n":"project_links: setup, link 1 of 5","as":"alice","sql":"insert into public.project_links (project_id, label, url) values ((select id from public.projects where name='Jeu indé solo' and owner_id='{alice}'), 'Lien 1', 'https://exemple.fr/1')","expect":"rows=1"},
    {"n":"project_links: setup, link 2 of 5","as":"alice","sql":"insert into public.project_links (project_id, label, url) values ((select id from public.projects where name='Jeu indé solo' and owner_id='{alice}'), 'Lien 2', 'https://exemple.fr/2')","expect":"rows=1"},
    {"n":"project_links: setup, link 3 of 5","as":"alice","sql":"insert into public.project_links (project_id, label, url) values ((select id from public.projects where name='Jeu indé solo' and owner_id='{alice}'), 'Lien 3', 'https://exemple.fr/3')","expect":"rows=1"},
    {"n":"project_links: setup, link 4 of 5","as":"alice","sql":"insert into public.project_links (project_id, label, url) values ((select id from public.projects where name='Jeu indé solo' and owner_id='{alice}'), 'Lien 4', 'https://exemple.fr/4')","expect":"rows=1"},
    {"n":"project_links: setup, link 5 of 5","as":"alice","sql":"insert into public.project_links (project_id, label, url) values ((select id from public.projects where name='Jeu indé solo' and owner_id='{alice}'), 'Lien 5', 'https://exemple.fr/5')","expect":"rows=1"},
    {"n":"project_links: a 6th link is refused (max 5 per project)","as":"alice","sql":"insert into public.project_links (project_id, label, url) values ((select id from public.projects where name='Jeu indé solo' and owner_id='{alice}'), 'Lien 6', 'https://exemple.fr/6')","expect":"error=at most 5 links"},

    {"n":"visibility: setup: create nadia, omar, petra, quinn","as":"admin","sql":"insert into auth.users (id, aud, role, email) values ('{nadia}','authenticated','authenticated','nadia@test.invalid'), ('{omar}','authenticated','authenticated','omar@test.invalid'), ('{petra}','authenticated','authenticated','petra@test.invalid'), ('{quinn}','authenticated','authenticated','quinn@test.invalid')","expect":"rows=4"},
    {"n":"visibility: onboarding cannot complete without choosing who can see the profile","as":"quinn","sql":"update public.profiles set display_name='Quinn', work_modes=array['remote'], country='FR', idea_statuses=array['exploring'], weekly_hours=array['5_10'], ambitions=array['for_fun'], onboarded=true where id='{quinn}'","expect":"error=profiles_onboarded_complete"},
    {"n":"visibility: an invalid value is refused","as":"quinn","sql":"update public.profiles set visibility='everyone' where id='{quinn}'","expect":"error=profiles_visibility_check"},
    {"n":"visibility: nadia onboards, private","as":"nadia","sql":"update public.profiles set display_name='Nadia', work_modes=array['remote'], country='FR', idea_statuses=array['exploring'], weekly_hours=array['5_10'], ambitions=array['for_fun'], visibility='private', onboarded=true where id='{nadia}'","expect":"rows=1"},
    {"n":"visibility: omar onboards, members","as":"omar","sql":"update public.profiles set display_name='Omar', work_modes=array['remote'], country='FR', idea_statuses=array['exploring'], weekly_hours=array['5_10'], ambitions=array['for_fun'], visibility='members', onboarded=true where id='{omar}'","expect":"rows=1"},
    {"n":"visibility: petra onboards, public, indexable, open to partners","as":"petra","sql":"update public.profiles set display_name='Petra', work_modes=array['remote'], country='FR', idea_statuses=array['exploring'], weekly_hours=array['5_10'], ambitions=array['for_fun'], visibility='public', search_indexable=true, open_to_partners=true, onboarded=true where id='{petra}'","expect":"rows=1"},
    {"n":"visibility: a slug was assigned to each, starting with their first name","as":"admin","sql":"select 1 from public.profiles where id='{nadia}' and slug like 'nadia-%' union all select 1 from public.profiles where id='{omar}' and slug like 'omar-%' union all select 1 from public.profiles where id='{petra}' and slug like 'petra-%'","expect":"rows=3"},
    {"n":"visibility: setup: give nadia/omar/petra fixed slugs so the tests below can use them directly (a stranger cannot look up someone else's slug themselves, by design)","as":"admin","sql":"update public.profiles set slug = case id when '{nadia}' then 'nadia-test' when '{omar}' then 'omar-test' when '{petra}' then 'petra-test' end where id in ('{nadia}','{omar}','{petra}')","expect":"rows=3"},
    {"n":"visibility: the backfill reached every existing profile (none was left null); a real account may have since picked a level other than private, which is fine and expected","as":"admin","sql":"select 1 from public.profiles where visibility is null and onboarded = true and id not in ('{quinn}')","expect":"rows=0"},
    {"n":"visibility: the slug cannot be changed, not even by the owner","as":"nadia","sql":"update public.profiles set slug='hacked' where id='{nadia}'","expect":"error=permission denied"},
    {"n":"visibility: page_views cannot be set directly, not even by the owner","as":"petra","sql":"update public.profiles set page_views=1000 where id='{petra}'","expect":"error=permission denied"},
    {"n":"visibility: the owner can change their own level","as":"omar","sql":"update public.profiles set search_indexable=true where id='{omar}'","expect":"rows=1"},
    {"n":"visibility: a stranger cannot change someone else's level","as":"bob","sql":"update public.profiles set visibility='public' where id='{omar}'","expect":"rows=0"},
    {"n":"visibility: the owner always sees their own page, even private","as":"nadia","sql":"select 1 from public.get_public_profile('nadia-test')","expect":"rows=1"},
    {"n":"visibility: a stranger cannot see a private profile","as":"bob","sql":"select 1 from public.get_public_profile('nadia-test')","expect":"rows=0"},
    {"n":"visibility: an unknown slug returns nothing","as":"bob","sql":"select 1 from public.get_public_profile('does-not-exist')","expect":"rows=0"},
    {"n":"visibility: a logged-out visitor cannot see a members-level profile","as":"anon","sql":"select 1 from public.get_public_profile('omar-test')","expect":"rows=0"},
    {"n":"visibility: any logged-in stranger can see a members-level profile","as":"bob","sql":"select 1 from public.get_public_profile('omar-test')","expect":"rows=1"},
    {"n":"visibility: a logged-out visitor can see a public profile","as":"anon","sql":"select 1 from public.get_public_profile('petra-test')","expect":"rows=1"},
    {"n":"visibility: a logged-in stranger can see a public profile too","as":"bob","sql":"select 1 from public.get_public_profile('petra-test')","expect":"rows=1"},
    {"n":"visibility: the public page carries the open-to-partners and indexable flags","as":"bob","sql":"select 1 from public.get_public_profile('petra-test') where open_to_partners = true and search_indexable = true","expect":"rows=1"},
    {"n":"visibility: the raw profiles table stays closed to a non-match stranger even at members level (no bulk listing)","as":"bob","sql":"select 1 from public.profiles where id='{omar}'","expect":"rows=0"},
    {"n":"visibility: ...and at public level too","as":"bob","sql":"select 1 from public.profiles where id='{petra}'","expect":"rows=0"},
    {"n":"visibility: setup: petra has a public and a private project","as":"petra","sql":"insert into public.projects (owner_id, name, category_id, started_on, ended_on, outcome, lessons, visibility) select '{petra}', 'Public one', (select id from public.categories where slug='video_games'), '2024-01-01', '2024-06-01', 'launched_then_stopped', 'Leçon.', 'public'","expect":"rows=1"},
    {"n":"visibility: ...","as":"petra","sql":"insert into public.projects (owner_id, name, category_id, started_on, ended_on, outcome, lessons, visibility) select '{petra}', 'Private one', (select id from public.categories where slug='video_games'), '2024-01-01', '2024-06-01', 'launched_then_stopped', 'Leçon.', 'private'","expect":"rows=1"},
    {"n":"visibility: setup: omar has a public project","as":"omar","sql":"insert into public.projects (owner_id, name, category_id, started_on, ended_on, outcome, lessons, visibility) select '{omar}', 'Omar public', (select id from public.categories where slug='video_games'), '2024-01-01', '2024-06-01', 'launched_then_stopped', 'Leçon.', 'public'","expect":"rows=1"},
    {"n":"visibility: setup: nadia has a public project","as":"nadia","sql":"insert into public.projects (owner_id, name, category_id, started_on, ended_on, outcome, lessons, visibility) select '{nadia}', 'Nadia public', (select id from public.categories where slug='video_games'), '2024-01-01', '2024-06-01', 'launched_then_stopped', 'Leçon.', 'public'","expect":"rows=1"},
    {"n":"visibility: a logged-out visitor sees only petra's public project","as":"anon","sql":"select 1 from public.get_public_projects('petra-test')","expect":"rows=1"},
    {"n":"visibility: ...and it is the public one, never the private one","as":"anon","sql":"select 1 from public.get_public_projects('petra-test') where name='Public one'","expect":"rows=1"},
    {"n":"visibility: a private project never shows even to a stranger who could see the profile","as":"bob","sql":"select 1 from public.get_public_projects('petra-test') where name='Private one'","expect":"rows=0"},
    {"n":"visibility: omar's public project follows his members level: hidden from a logged-out visitor","as":"anon","sql":"select 1 from public.get_public_projects('omar-test')","expect":"rows=0"},
    {"n":"visibility: ...shown to a logged-in stranger","as":"bob","sql":"select 1 from public.get_public_projects('omar-test')","expect":"rows=1"},
    {"n":"visibility: nadia's public project follows her private level: hidden from a stranger","as":"bob","sql":"select 1 from public.get_public_projects('nadia-test')","expect":"rows=0"},
    {"n":"visibility: ...but nadia herself sees it","as":"nadia","sql":"select 1 from public.get_public_projects('nadia-test')","expect":"rows=1"},
    {"n":"visibility: the owner's own visit is never counted","as":"petra","sql":"select public.increment_page_view('petra-test')","expect":"rows=1"},
    {"n":"visibility: ...page_views is still 0","as":"admin","sql":"select 1 from public.profiles where id='{petra}' and page_views=0","expect":"rows=1"},
    {"n":"visibility: a stranger's visit is counted","as":"bob","sql":"select public.increment_page_view('petra-test')","expect":"rows=1"},
    {"n":"visibility: ...page_views is now 1","as":"admin","sql":"select 1 from public.profiles where id='{petra}' and page_views=1","expect":"rows=1"},
    {"n":"visibility: a logged-out visitor's visit is counted too","as":"anon","sql":"select public.increment_page_view('petra-test')","expect":"rows=1"},
    {"n":"visibility: ...page_views is now 2","as":"admin","sql":"select 1 from public.profiles where id='{petra}' and page_views=2","expect":"rows=1"},
    {"n":"visibility: a visit to a private profile is never counted, no error either","as":"bob","sql":"select public.increment_page_view('nadia-test')","expect":"rows=1"},
    {"n":"visibility: ...page_views stayed 0","as":"admin","sql":"select 1 from public.profiles where id='{nadia}' and page_views=0","expect":"rows=1"},
    {"n":"visibility: an unknown slug does nothing, no error","as":"bob","sql":"select public.increment_page_view('does-not-exist')","expect":"rows=1"},
    {"n":"visibility: setup: bob blocks petra","as":"bob","sql":"insert into public.blocks (blocker_id, blocked_id) values ('{bob}','{petra}')","expect":"rows=1"},
    {"n":"visibility: a blocked, logged-in visitor no longer sees a public profile","as":"bob","sql":"select 1 from public.get_public_profile('petra-test')","expect":"rows=0"},
    {"n":"visibility: ...nor its projects","as":"bob","sql":"select 1 from public.get_public_projects('petra-test')","expect":"rows=0"},
    {"n":"visibility: ...nor does their visit count","as":"bob","sql":"select public.increment_page_view('petra-test')","expect":"rows=1"},
    {"n":"visibility: ...page_views still 2, the blocked visit was not counted","as":"admin","sql":"select 1 from public.profiles where id='{petra}' and page_views=2","expect":"rows=1"},
    {"n":"visibility: a public profile stays visible to a logged-out visitor even after a block exists","as":"anon","sql":"select 1 from public.get_public_profile('petra-test')","expect":"rows=1"},
    ...
    {"n":"account deletion: her projects are gone","as":"admin","sql":"select 1 from public.projects where owner_id='{alice}'","expect":"rows=0"},
    {"n":"account deletion: her projects' links are gone too","as":"admin","sql":"select 1 from public.project_links pl join public.projects p on p.id = pl.project_id where p.owner_id='{alice}'","expect":"rows=0"},
