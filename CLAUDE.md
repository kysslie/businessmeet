...
- **No bulk listing** (Elie, 2026-09-22): nobody reads a whole table of other people's rows directly, and no RLS policy is ever widened to allow that. Discovery of anyone but yourself always goes through a narrow, single-purpose `SECURITY DEFINER` function: one candidate at a time (`get_feed()`, max 20 per call) or one slug at a time (`get_public_profile()`/`get_public_projects()`, see P3). Any future browsing/listing feature (see Backlog, "community section") must follow the same shape: a paginated function with a hard limit, never a raw table grant.
...
profiles            id (PK, = auth.users.id), display_name, avatar_path,
                    ...
                    visibility ('private'|'members'|'public', no default, required to onboard),
                    slug (unique, auto-assigned once, never editable by anyone),
                    open_to_partners (bool), search_indexable (bool), page_views (int),
...
### Who can see a profile (P3, built 2026-09-22, migration `20260922110000_portfolio_visibility.sql`)

Elie amended the original "is_public boolean" plan to a three-level `profiles.visibility` after asking what the rule already was (see Decision Log). One function, `portfolio_visible_to(owner, visibility)`, is the single place this is decided, called by everything else below:

- **The owner** always sees their own page, at every level (lets them preview before publishing).
- **`private`** (no default; existing accounts were backfilled to this, nothing changed for them): only the owner, a match, or a shared Parcours — the same rule as before P3, untouched.
- **`members`**: any logged-in visitor, unless a block exists between the two (either direction).
- **`public`**: anyone, including logged out; a block only applies to a visitor we can identify, so a public profile stays visible to a logged-out visitor even if that person blocked (or was blocked by) the owner while signed in (this limit is explained on the block-confirmation screen).

The raw `profiles` table's own RLS policy was **not** widened to cover `members`/`public` — every non-match read of someone else's profile goes through `get_public_profile(slug)` / `get_public_projects(slug)` (both `SECURITY DEFINER`, both call `portfolio_visible_to()`), the same shape as `get_feed()`. This is what makes the "no bulk listing" rule (see Architecture) true by construction: there is no table policy a future feature could accidentally query in bulk against, only a slug-at-a-time function. A public project (P2's `projects.visibility = 'public'`) only actually shows if the owning profile also passes `portfolio_visible_to()` for that viewer — a private project never shows to anyone but the owner, full stop, regardless of the profile's level.

`slug`: assigned once, by a trigger, the moment onboarding completes (first word of the display name + 4 random characters, retried on collision); nobody has an UPDATE grant on it, not even the owner. `page_views`: same, no UPDATE grant, moves only through `increment_page_view(slug)`, which never counts the owner and never counts a visit that `portfolio_visible_to()` would have refused anyway.

`open_to_partners`: stored now (a toggle on `/profile`), does nothing yet. Once matching returns (see Backlog, "Phase 2 — matching redesign"), it is meant to be the flag that lets a profile appear as a candidate regardless of its visibility level — it never changes the visibility level itself.
...
| P3 | **Public portfolio page (built 2026-09-22).** `profiles` gets `visibility` (private/members/public, amended from a plain public/private switch — see Decision Log), `slug`, `search_indexable`, `open_to_partners`, `page_views`; route `/p/[slug]`; toggles + "Copier le lien" on the profile; view counter; a project also needs its own P2 visibility set to public | A public profile's page loads for a logged-out visitor with only public info; a private one 404s; a non-indexable one carries `noindex` (not yet confirmed by Elie) |
| P3b | **Share card (built 2026-09-23).** Open Graph image per public `/p/[slug]` page (1200×630: display name, city, project count, counts by outcome, "Ouvert aux associés" badge if on; public data only, `next/og` `ImageResponse`, no new dependency). "Télécharger ma carte" button producing a 1080×1080 square and a 1200×630 landscape PNG. A one-time prompt after a user's first public project: "Ton portfolio est en ligne. Partage-le à quelqu'un qui devrait le voir." with a copy-link button | The OG image shows correctly when the public link is pasted into a chat app (verified: the `og:image` meta tag and the image itself both render); both downloaded PNGs look right; the one-time prompt appears once and not again (not yet confirmed by Elie) |
| P4 | **Metrics for the new milestone (built 2026-09-22)**: `metrics_active_profiles` kept as-is; new `metrics_project_adoption`, `metrics_public_pages`, both admin-only, excluding `is_demo`; the two old match-only views (`metrics_weekly_matches`, `metrics_retention_30d`) left in place, not dropped, just not part of today's three | Exact SQL given to Elie; each view returns sensible numbers |
| P5 | *(was F8, built 2026-09-23)* Account deletion (cascades projects and links too), **French privacy page** (with the acquisition clause, marked "pending legal review") | Deleting an account removes all of that user's rows including projects/links; the privacy page is live; DEBT-021 BLOCKER clears (not yet confirmed by Elie) |
| F9 | PWA install (manifest, icons), production check, full smoke test (built 2026-09-23) | The app installs on a phone home screen; the whole flow works in production |
...
- **P3b — Share card (built 2026-09-23, migration `20260923110000_share_prompt.sql`; one new column, additive, no existing data touched).** `share_prompt_shown` on `profiles` tracks the one-time prompt. Three image routes, all built from a shared `loadShareCardData(slug)` helper that calls the exact same `get_public_profile`/`get_public_projects` functions the portfolio page itself uses — so a private or members-only profile can never leak into a shared image, since a link-preview bot has no session, same as any other stranger: `/p/[slug]/opengraph-image` (Next's auto-embedded convention, 1200×630), and two explicit downloads behind "Télécharger ma carte" on `/profile`, `/p/[slug]/card-landscape.png` (same 1200×630) and `/p/[slug]/card-square.png` (1080×1080), both served with `Content-Disposition: attachment` so they actually download rather than open in a tab. Only non-zero outcome counts are shown (no "0 revendus" clutter). The one-time prompt shows when a profile has at least one public project **and** its own visibility is Membres or Public (not Privé — nothing would actually be shareable otherwise, a refinement on Elie's spec I flagged rather than guessed silently); it marks itself shown the instant it renders, not on dismiss, via a small server action, so it never reappears regardless of what the person does with it. Browser-tested by me with a throwaway public account with two projects: the `og:image` meta tag and the image itself both render correctly with the right name/place/badge/counts; both download routes return real PNGs with the right filename and `Content-Disposition`; the square card visually confirmed (mark centered, nothing clipped); the prompt appeared on first load, dismissing it hid it, and reloading confirmed it never came back. 456/456 database checks still pass. NOT yet confirmed by Elie.

- **F9 — PWA install, production check (built 2026-09-23; part of the planned build order: F0–F3, P1–P5, P3b, F9, now all built).** `manifest.ts` + generated icon routes (`/icon-192.png`, `/icon-512.png`, `/icon-512-maskable.png`, plus `/icon` and `/apple-icon` via Next.js's own conventions) using `next/og`'s `ImageResponse` — a placeholder "BM" mark in the site's own colors, no real branding yet (DEBT-036). `theme-color` and Apple web-app meta tags added. Found and fixed a real bug: the icon/apple-icon routes have no file extension in their URL, so `src/proxy.ts`'s middleware matcher was treating them as protected pages and redirecting logged-out requests (including the browser's own icon fetches) to `/login` — fixed by naming them in the matcher's exclusion, the same way `favicon.ico` already was. Also removed the five unused scaffold SVGs from the original setup (DEBT-007). Verified in the browser (manifest, all icons load, protected pages still redirect correctly) and again on the live production URL after deploy: manifest and every icon route return the right content type and status. NOT yet confirmed by Elie (installing it on an actual phone is his to try).

- **P5 — Account deletion and the French privacy page (built 2026-09-23; no migration — every foreign key pointing at a user was already `ON DELETE CASCADE`, so deletion needed only app code).** Checked the blocker first: DEBT-008 (secret key rejected in September) is **resolved** — verified with a raw admin-API call before building anything on top of it. `src/lib/supabase/admin.ts` deletes the auth user via a plain `fetch` to Supabase's admin REST API rather than the `@supabase/supabase-js` client, which always tries to open a realtime (WebSocket) connection and crashes on plain Node 20 (no native WebSocket) even when realtime is never used — a direct fetch sidesteps that with no new dependency. `/settings` gets "Supprimer mon compte": asks first, then requires the current password (same check as changing a password), removes the person's Storage photos (best-effort, never blocks the deletion itself), calls the admin API, signs out, redirects to `/?deleted=1` with a confirmation banner. `/privacy` is a full French draft (data collected, why, who can see it, sub-processors, cookies, retention, GDPR rights, how to delete, the acquisition clause, an 18+ statement not yet technically enforced, how changes are announced) — clearly marked as a draft pending legal review, matching how the acquisition clause was already handled; two contact-email spots are left as visible placeholders for Elie to fill in, not invented (DEBT-035). Browser-tested by me with a throwaway account that had a project: wrong password correctly refused first, then deleted with the right one; confirmed at the database level that the auth user, profile and project were all gone (not just app-visible); confirmed the session was actually cleared (`/profile` redirected to login afterward); confirmed `/privacy` reads logged out. 456/456 database checks still pass (no schema touched). Real data unaffected throughout. NOT yet confirmed by Elie — and this is real: once he tries "Supprimer mon compte" on any account, including his own, it is genuinely gone.

- **P4 — Metrics for the new milestone (built 2026-09-22, migration `20260923100000_portfolio_metrics.sql`; two new views only, nothing dropped or altered).** `metrics_project_adoption` (per sign-up week: how many signed up, how many have added at least one project, and the percentage) and `metrics_public_pages` (how many profiles are currently Public, and the total view count across everyone's page). `metrics_active_profiles` needed no change. Same lockdown as the existing views (`security_invoker`, no grant to anon/authenticated). Verified with real data: 2 real sign-ups that week, 0% with a project yet; 1 public profile (John), 1 page view — matching what actually happened in this session. `db advisors`: no new warnings. Test script: fixed one test that had hardcoded a snapshot of live data (it asserted 3 specific real accounts were still `private`, which broke the moment Elie set John's to `public` — replaced with a check that the backfill reached every profile, not what any one of them currently says) — 456/456 after the fix. NOT yet confirmed by Elie (he reads these himself from the Supabase SQL editor, not through the app).

- **P3 — Public portfolio page (built 2026-09-22, migrations `20260922110000_portfolio_visibility.sql`, `20260922120000_backfill_slugs.sql`, `20260922130000_fix_slug_case_bug.sql`).** Elie amended the plan mid-build to a three-level `profiles.visibility` (private/members/public, no default — see "Who can see a profile" in section 5 and the Decision Log) after asking what the existing rule even was. Existing accounts backfilled to `private` (no change in who sees them). Two bugs found and fixed with follow-up migrations before I moved on: the visibility backfill ran before the slug-assigning trigger existed (3 real accounts got no slug — fixed); the slug generator lowercased *after* stripping non-a-z0-9 characters, so every name's first (capitalized) letter was silently dropped ("John" → "ohn-...") — fixed and reassigned. `db advisors`: 3 new warnings, all expected (the 3 functions anon/authenticated need to call). Test script grew from 407 to 456 checks (every tier × anon/stranger/match/blocked/owner, projects following the profile's level, the raw table staying closed even at `public` level, page-view counting rules). App layer: the visibility question moved into onboarding (`ProfileForm`, required, no option pre-checked); "Votre page publique" section on `/profile` (toggles, copy-link, view count); `/p/[slug]` (public page, `noindex` when not indexable, links `rel="noopener noreferrer nofollow"`); P2's project form gained the "Rendre ce projet public" checkbox it was always going to need (I had deferred it to P3, per the plan). Browser-tested by me end to end with a throwaway account: full onboarding including picking "Public", the page renders logged in AND in a second, cookie-cleared "logged out" context, a project only appears once marked public, the view counter moved for the logged-out visit but not the owner's own, the `noindex` tag appeared and then disappeared after toggling search-indexing. Real data unaffected throughout (4 accounts, 1 match, 1 message, 0 projects at the end). **Confirmed by Elie 2026-09-22**: set John's real account to Public, opened the link himself, it showed his card. Content ("what we put in it") to be filled in later.
...
**Every planned build-plan item is now built: F0–F3, P1–P5, P3b, F9.** What's left: the French category migration (`supabase/proposed/`, still awaiting Elie's yes), Elie's own pending tests (see below and the memory file), and the Backlog (Phase 2 matching redesign, native app, community section, launch-week items). Elie still needs to fill in the two "[adresse à compléter]" placeholders on the privacy page (DEBT-035) and give it a real legal review before public launch (DEBT-004/020/022).
...
**Metrics: how Elie reads them.** In the Supabase dashboard open **SQL Editor**, paste one line and press Run (the views cannot be read by the app or by users). The three for today's milestone:
```sql
select * from public.metrics_active_profiles;
select * from public.metrics_project_adoption order by signup_week_start desc;
select * from public.metrics_public_pages;
```
...
- `metrics_project_adoption`: per sign-up week, how many signed up, how many have added at least one project (public or private), and the percentage.
- `metrics_public_pages`: how many profiles are set to Public right now, and the total view count across everyone's page.

Two older views from before the pivot still work but aren't part of today's three (kept, not dropped, since matching is only parked, not gone):
```sql
...
- 2026-09-22 | **P3 amended mid-plan: `visibility` enum, not `is_public` boolean** (Elie, after asking "what is the current who-can-see-what rule?"): three levels — `private` (owner, matches, shared Parcours — the unchanged existing rule), `members` (any logged-in visitor), `public` (anyone, including logged out); no default, so onboarding must make an explicit choice (extends `profiles_onboarded_complete`, the same style as every other required field); existing accounts backfilled to `private`; a block overrides every level for a logged-in viewer, but a logged-out visitor cannot be identified so a block cannot apply to them (noted on the block-confirmation screen); `open_to_partners` is unrelated to this ladder — it will gate `get_feed()` eligibility once matching returns, never the visibility level itself | Keeping the simple public/private switch | Elie's call, once he saw the existing rule was already more layered ("matches and Parcours can already see a private profile") than a plain on/off switch would express
- 2026-09-22 | **P4 metrics: keep the old match views, add two new ones, drop nothing** (Claude's design): `metrics_project_adoption` and `metrics_public_pages` are additive; `metrics_weekly_matches`/`metrics_retention_30d` (decision D) stay in place since matching is only parked, not gone, and dropping a working view for no reason isn't worth the ceremony of a destructive migration | Dropping the two old match views as decision D's spec literally implied ("replaces decision D's match metrics") | Nothing is lost either way; leaving them costs nothing and keeps the option open
- 2026-09-23 | **P5 account deletion: a raw fetch to the admin API, not the supabase-js client** (Claude's design, flagged rather than decided silently): `@supabase/supabase-js`'s `createClient()` always tries to open a realtime (WebSocket) connection and crashes on plain Node 20, which has no native WebSocket, even though deleting a user never touches realtime | Adding the `ws` package so the full client works (Vercel's Node runtime might not even need it) | No new dependency; a single DELETE call doesn't need the whole client just to reach the admin API
- 2026-09-23 | **P5 privacy page: a full draft now, not a placeholder** (Claude, following the same rule already applied to the acquisition clause): written and published to `/privacy` marked "brouillon, en attente de relecture juridique", covering data collected, purposes, who can see what, sub-processors, cookies, retention, GDPR rights, deletion, the acquisition clause, an 18+ statement (not technically enforced), and how changes are announced. Two contact-email spots left as visible placeholders rather than invented | Waiting for real legal text before publishing anything | Elie's own rule for the acquisition clause: ship a clearly-marked draft now, get it properly reviewed before public launch (DEBT-004/020/022)
- 2026-09-23 | **P3b share prompt: also requires the profile itself to be visible, not just a public project** (Claude's design, flagged rather than decided silently): the prompt only shows when the profile's own `visibility` is `members` or `public`, in addition to having a public project — if the profile is still `private`, the project being marked public does not actually make the page visible to anyone, so telling the person to "share it with someone who should see it" would be misleading | Triggering the prompt on the project alone, exactly as worded | A prompt that tells someone to share a page nobody else can open would be confusing and undermine trust in the feature
...
- [LOW] DEBT-033 `/p/[slug]` tries to build a signed link for the profile photo the normal way (`storage.createSignedUrl`), which needs read access on the `avatars` bucket. That bucket's policy is "any logged-in user may read" (F3) — it was never extended for logged-out visitors. Untested whether a logged-out visitor actually gets a photo on a public page (the code degrades to the 🙂 placeholder on failure, so nothing breaks either way, but I did not verify which one happens). Check, and if photos are missing for logged-out visitors, add a storage policy: anyone may read a photo whose owning profile is currently `visibility = 'public'`.
- [LOW] DEBT-034 `/p/[slug]`'s view counter moves on every page load that passes `portfolio_visible_to()`, including a browser reload or a link-preview bot fetching Open Graph data (P3b) — it is a hit counter, not unique visitors. Matches the original ask ("increments... stores no visitor data"); revisit if the number needs to mean something more precise later.
...
- [HIGH] DEBT-019 Two demo accounts exist (`demo.ana@demo.invalid`, `demo.marco@demo.invalid`, see Demo accounts) with passwords known to Elie and unverified fake addresses. Delete them before public launch, together with any other test accounts — the "Supprimer mon compte" flow (P5) now works and removes their Storage photo too, so this can be done from the app itself (log in as each demo account and use it), no SQL needed any more.
...
- **RESOLVED 2026-09-23** ~~[BLOCKER] DEBT-021~~ **No public sign-up and no launch before the privacy page is live** (Elie, 2026-09-20). `/privacy` is live and account deletion works (P5, 2026-09-23), so the blocking condition is met. DEBT-004/020/022 (real legal review) are still open and should happen before actually promoting the site.
- [HIGH] DEBT-022 The privacy page (`/privacy`, P5) is now a full draft — still needs a real legal review before public launch, and Elie needs to fill in the two contact-email placeholders (DEBT-035). See DEBT-004 and DEBT-020.
- [LOW] DEBT-035 `/privacy` has two "[adresse à compléter par Elie]" placeholders (responsible party / contact) where a real contact e-mail belongs — deliberately left as a placeholder rather than invented. Fill in before public launch.
- [LOW] DEBT-036 The PWA install icon (F9) is a generated placeholder — a "BM" mark on the site's own dark color, not real branding. Swap `src/app/icon-192.png/route.tsx`, `icon-512.png/route.tsx`, `icon-512-maskable.png/route.tsx`, `icon.tsx` and `apple-icon.tsx` for a real logo whenever one exists.
...
- **RESOLVED 2026-09-23** ~~[HIGH] DEBT-008~~ `SUPABASE_SECRET_KEY` in `.env.local` was rejected (HTTP 401) by the Supabase admin API on 2026-09-19. Re-tested directly before building P5 (a raw admin-API call, no key printed): status 200, working. Cause of the original 401 still unknown, but it is not blocking account deletion any more.
