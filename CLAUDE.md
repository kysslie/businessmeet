...
**Next planned step:** P2 — projects (`projects` + `project_links` tables and RLS, "Mes projets" CRUD on the profile). Will present the migration plan and wait for Elie's OK before applying it (it's a new table, not an existing-data change, but still a database-structure change per his rule). The French category migration (`supabase/proposed/`) and Elie's matching-related tests stay pending but are no longer blocking — they can wait until matching is re-enabled.
