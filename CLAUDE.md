...
**Next planned step:** F9 — PWA install (manifest, icons), production check, full smoke test. The French category migration (`supabase/proposed/`) and Elie's matching-related tests stay pending but are no longer blocking — they can wait until matching is re-enabled. Elie still needs to fill in the two "[adresse à compléter]" placeholders on the privacy page (DEBT-035) and give the page a real legal review before public launch (DEBT-004/020/022).
