...
- 2026-09-19 | F4 feed: `get_feed()` implements the section 4 rules literally (the viewer's own work mode decides: Remote OK sees everyone in shared active categories, Local only sees the same city, compared trimmed and case-insensitively); random order, max 20 per batch; UI = one card at a time, drag or ✕/♥ buttons; swipes are permanent | Ranked feed; undo | Ship first, rank later (per brief). OPEN QUESTION for Elie: literal rule lets a Remote OK person see a Local-only person in another city, who can never see them back, so a like there can never become a match. Suggested tweak (one-line change): show a candidate only if the pair is compatible both ways (both Remote OK, or same city).
