...
**Next planned step:** P4 — metrics for the new milestone (`metrics_active_profiles` kept, `metrics_project_adoption`, `metrics_public_pages`, admin-only, excluding `is_demo`). Will present the plan and wait for Elie's OK. The French category migration (`supabase/proposed/`) and Elie's matching-related tests stay pending but are no longer blocking — they can wait until matching is re-enabled.
